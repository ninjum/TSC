...
ExternalProject_Add(
  ...
  # cegui-stb-greyscale.patch: the STB codec decodes a greyscale PNG to ONE
  # channel and greyscale+alpha to TWO, then rejects both because CEGUI's
  # Texture has no pixel format for them - "Invalid image format. Only RGB and
  # RGBA images are supported". 38 of the game's 1029 PNGs are greyscale, and
  # each one the GUI touched printed a CEGUI::RendererException and drew a dummy
  # image instead. The patch asks stb to widen those two cases to RGBA, which is
  # the same decoder doing the conversion the caller would otherwise have to do,
  # and leaves 3- and 4-channel images untouched. Converting the assets would
  # have fixed these 38 files only; this fixes any greyscale PNG, including one
  # in somebody's own level.
  # cegui-std-regex.patch: CEGUI's editbox validation is PCRE-only, and
  # System::createRegexMatcher() returns 0 when CEGUI is built without PCRE. An
  # Editbox with no validator THROWS from setValidationString - which every
  # Spinner calls - so a PCRE-less build could not show a spinner at all: the
  # 2.2.0-beta2 AppImage aborted the moment the Options menu was opened, on
  # game_spinner_camera_hor_speed. The patch adds a std::regex matcher and
  # returns it on that path, so validation works with no external library.
  PATCH_COMMAND patch -p1 < "${TSC_SOURCE_DIR}/../cegui-cpp11.patch" && patch -p1 < "${TSC_SOURCE_DIR}/../cegui-cmake-policy.patch" && patch -p1 < "${TSC_SOURCE_DIR}/../cegui-stb-greyscale.patch" && patch -p1 < "${TSC_SOURCE_DIR}/../cegui-std-regex.patch"
  LIST_SEPARATOR |
